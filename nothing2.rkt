...
(struct outward-arg (index) #:prefab)
...
(define (compile-procesure arg-names body-exp env)
  ...
  (define-values (temp-count final-instrs) (allocate-registers instrs))
  ...
  (append
   ...
   (append-map (match-lambda
		...
		[`(,(and op (or 'call 'tailcall)) ,target ,label (,arg ...))
		 ;; TODO: prepare for tailcalls by figuring out what
		 ;; the maximum tail argc is and if it's different to
		 ;; the current procedure's argc, emit an instruction
		 ;; that moves the return address and frame before
		 ;; storing over the inward-args.
		 ;;
		 ;; TODO: prepare for nontail calls by moving the
		 ;; stack pointer an appropriate number of bytes
		 ;; before the call, and depending on calling
		 ;; convention moving it back again after the call
		 ;; returns. Obviously tailcalls don't work if the
		 ;; caller has to clean up the args.
		 ;;
		 (define argcount (length arg))
		 (define calltype (if (eq? op 'tailcall) 'tail 'nontail))
		 (append (list `(prepare-call ,calltype ,argcount))
			 (do ((i 0 (+ i 1))
			      (arg arg (cdr arg))
			      (acc '() (cons `(move-word ,(outward-arg i) ,(car arg)) acc)))
			     ((null? arg) (reverse acc)))
			 (list `(,op ,(preg 'eax) ,label ,(map outward-arg (iota argcount)))
...
(define (expand-temporary-loads-and-stores instrs)
  (define (shuffle-for-two-args make-instr target s1 s2)
    (cond
     [(and (equal? target s1) (not (and (temporary? s1) (temporary? s2))))
  ...
  (append-map (match-lambda
	       [(and i `(move-word ,(temporary n) ,(temporary m)))
		(if (= n m)
		    (list i)
		    (let ((r (fresh-reg)))
		      (list `(move-word ,r ,(temporary m))
...
(define (assemble-instr i)
  (match i
    [_ (error 'assemble-instr "Cannot assemble ~v" i)]))

(define (assemble instrs)
  (list->bytes (flatten (map assemble-instr instrs))))
...
(define (te args exp env)
  ...
  (define assembled (assemble final-instrs))
